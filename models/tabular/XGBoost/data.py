...
def get_data(path: str) -> pd.DataFrame:
    ...
    df_primary = pd.read_csv(f"{path}/table_dataset/primary_data.csv", sep=";")
    df_secondary = pd.read_csv(
        f"{path}/table_dataset/secondary_data.csv", sep=";")
    df_mushnames = pd.read_csv(
        f"{path}/table_dataset/species_names.csv", sep=";")

    # Species name from primary, add to secondary
    primary_name_df = df_primary[['family', 'name']]
    df_primname_rep = primary_name_df.loc[primary_name_df.index.repeat(
        353)].reset_index(drop=True)

    # Clean names
    data_secondary_labelled = pd.concat(
        [df_secondary, df_primname_rep], axis=1)
    data_secondary_labelled['family'] = data_secondary_labelled['family'].str.replace(
        " Family", "", regex=False)
    data_secondary_labelled['Common Name'] = data_secondary_labelled["family"] + \
        " " + data_secondary_labelled["name"]

    # Merge to scientific names
    data_merge_scname = data_secondary_labelled.merge(
        df_mushnames, how='left', on='Common Name')
    data_tabular_final = data_merge_scname.drop(
        columns=['family', 'name', 'Common Name'])
    data_tabular_final.columns = data_tabular_final.columns.str.replace(
        '-', '_').str.replace(' ', '_').str.lower()
    data_tabular_final["class"] = (
        data_tabular_final["class"] == "p").astype(int)
...
def get_data_reduced(path: str) -> pd.DataFrame:
    """
    ...
    df_concat['scientific_name'] = df_concat['scientific_name']\
    ...
    data_tabular_image = get_data(path).merge(
        df_concat, how='inner', on='scientific_name')

    print("✅ Reduced tabular data loaded and cleaned, shape:",
          data_tabular_image.shape)
